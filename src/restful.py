...
class realtimeHandler(basic_handler.BaseHandler):
    ...
    def get(self, *args):
        ...
        if groupname is not None:
            ...
            self.render("test2.html", svg="/static/svg/bpr.min.svg",
                        groupname=groupname, title=title, tagnames=tagnames)
        else:
            self.write('<center>The group is not esxit!<a href="/">Go Home</a></center>')

    def get_tags(self, taggroup):

        tags = [taggroup + '.TS', taggroup + '.NAME',
                taggroup + '.GY', taggroup + '.DY', taggroup + '.MB']
        return tags
...
class Application(tornado.web.Application):
    ...
    def __init__(self):
        handlers = [
            ...
            (r'/ws/v1', api_handler.WebSocketHandler),
